 ...
 *
 * Called before parseCardBlocks() — acts as a pre-processor on finalAnswer.
 */
...
function looksLikeD2(text: string): boolean {
  ...
  const identifierLines = lines.filter((l) => /^[\w一-鿿][\w一-鿿\s]*[:{]/.test(l.trim()) || /->|<-/.test(l));
...
/**
 * Main entry point: apply all auto-upgrade passes in order.
 *
 * Pass 1: ```d2 fences → ```card:d2
 * Pass 2: bare D2 content (no fences) → ```card:d2
 * Pass 3: bare rich Markdown → ```card:markdown
 */
export function autoUpgradeCards(text: string): string {
