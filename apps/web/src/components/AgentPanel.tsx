...
const panelStyle: CSSProperties = {
  ...
  padding: "16px 12px", background: "#161b22",
  borderRight: "1px solid #30363d", height: "100%", overflow: "auto",
};
...
const selectStyle: CSSProperties = {
  width: "100%", padding: "6px 8px", background: "#21262d",
  border: "1px solid #30363d", borderRadius: 4, color: "#c9d1d9", outline: "none",
};
const textareaStyle: CSSProperties = {
  ...
  outline: "none", lineHeight: 1.5, minHeight: 120,
};
...
const stepsInputStyle: CSSProperties = {
  flex: 1, padding: "5px 8px", background: "#21262d",
  border: "1px solid #30363d", borderRadius: 4, color: "#c9d1d9",
  fontFamily: "inherit", fontSize: 12, outline: "none", width: "100%",
};
const dividerStyle: CSSProperties = { borderColor: "#30363d", margin: "4px 0" };
const titleStyle: CSSProperties = { color: "#58a6ff", fontWeight: 700, fontSize: 16, letterSpacing: 1 };
...
function btnStyle(primary: boolean, danger = false): CSSProperties {
  return {
    ...
    color: "#fff", fontWeight: 600, fontSize: 12, transition: "background 0.15s",
  };
}
...
function providerDotStyle(provider: string): CSSProperties {
  return {
    ...
    background: PROVIDER_COLORS[provider] ?? "#8b949e", marginRight: 6,
  };
}

export function AgentPanel({
  config, onChange, task, onTaskChange, onSubmit, onAbort, isRunning,
}: AgentPanelProps) {
  return (
    <div style={panelStyle}>
      ...
      <div>
        ...
        <div style={{ marginTop: 4, fontSize: 11, color: "#8b949e" }}>
          <span style={providerDotStyle(MODELS.find((m) => m.id === config.modelId)?.provider ?? "")} />
      ...
      <div>
        ...
        <div style={stepsRowStyle}>
          <input
            ...
            onChange={(e) => onChange({ ...config, maxSteps: Math.max(1, Math.min(30, Number(e.target.value))) })}
          />
        </div>
      </div>
      ...
      <div style={rowStyle}>
        <button style={btnStyle(true)} onClick={onSubmit} disabled={isRunning || !task.trim()}>
          {isRunning ? "Running…" : "Run Agent"}
        </button>
        {isRunning && (
          <button style={btnStyle(false, true)} onClick={onAbort}>Stop</button>
        )}
      </div>
    </div>
  );
