...
async function exportPng(svgString: string, meta?: string) {
  ...
  // Sanitize SVG: remove XML declaration, ensure proper namespace
  const clean = svgString.startsWith("<?xml")
    ? svgString.slice(svgString.indexOf("<svg"))
    : svgString;

  // Parse natural SVG dimensions from viewBox or width/height attributes
  ...
  // Export at 2× resolution for crisp rendering on high-DPI screens
  const scale = 2;
  const exportW = Math.round(svgW * scale);
  const exportH = Math.round(svgH * scale);

  // Force explicit dimensions on the SVG so the image loads at the right size
  const sized = clean.replace(
    /<svg([^>]*)>/,
    `<svg$1 width="${exportW}" height="${exportH}">`
  );
...
export function D2Card({ content, meta, onRenderD2, className, style, fillHeight }: D2CardProps) {
  ...
  const containerStyle: CSSProperties = {
    ...CARD_STYLE,
    ...style,
    ...(fillHeight && { display: "flex", flexDirection: "column", flex: 1, height: "100%", margin: 0, borderRadius: 0, border: "none" }),
  };
    ...
    <div className={className} style={containerStyle}>
      ...
      {state.status === "ok" && svgSrcDoc && (
        <iframe
          ...
          style={{
            ...SVG_IFRAME_STYLE,
            // fillHeight: stretch to remaining parent height; otherwise use auto-measured height
            ...(fillHeight ? { flex: 1, height: "100%" } : { height: `${iframeHeight}px` }),
          }}
          title={title}
