...
async function exportDocx(content: string, meta?: string) {
  ...
  while (i < lines.length) {
    ...
    if (headingMatch) {
      ...
      children.push(
        new Paragraph({ heading: headingLevels[level] ?? HeadingLevel.HEADING_1, children: [new TextRun(text)] })
      );
      i++;
      continue;
    }
          ...
          children: cells.map(
            (text) =>
              new TableCell({
                children: [
                  new Paragraph({
                    children: [new TextRun({ text: text.replace(/\*\*(.+?)\*\*/g, "$1"), bold: isHeader })],
                  }),
                ],
  ...
  const runs: InstanceType<typeof TextRun>[] = [];
    ...
    while ((m = inlineRe.exec(line)) !== null) {
      ...
      else if (m[4] !== undefined) runs.push(new TextRun({ text: m[4], font: "Courier New", size: 18 }));
