/**
 * B4 — Approval policy for write-class tools.
 *
 * Wraps existing write tools (write_file, patch_file, delete_file,
 * rename_file) with a `needsApproval` function so the bscode HITL gate
 * fires on PATHS or SIZES that match the configured policy and skips
 * everything else. The Vercel AI SDK 6 pattern adapted to bscode's
 * agentkit-driven runtime.
 *
 * Defaults are conservative — the bare policy with NO rules is "approve
 * everything". A consumer adds policies until the gate matches their
 * threat model. Paths are matched as prefixes; size rules apply per
...
// ── Tool wrappers ────────────────────────────────────────────────────────────
//
// Each helper takes an existing ToolDefinition (built by the regular
// createWriteFileTool / createPatchFileTool / etc.) and returns a
// new ToolDefinition with `needsApproval` set to a policy-driven
...
function applyPolicy<I, O>(
  ...
  return {
    ...
    needsApproval: ((input: I) =>
      ...
      })) as ToolDefinition<I, O>["needsApproval"],
  };
}
...
/**
 * Convenience presets the host can mix and match. These are bsCode-product
 * choices, NOT generic agentkit ones — keep them here so the agentkit
 * core stays product-agnostic.
 */
export const PolicyPresets = {
  /** Nothing needs approval — useful for dev sandboxes. */
  permissive(): ApprovalPolicy {
    return new ApprovalPolicy({ rules: [], defaultVerdict: "allow" });
  },
  /** Production default — every write needs approval. */
  strict(): ApprovalPolicy {
    return new ApprovalPolicy({ rules: [], defaultVerdict: "require" });
  },
  /**
   * Sensible middle ground: dotfiles, env, and CI configs always require
   * approval; large bulk writes (>5 KB at once) require approval; small
   * source-file edits run free.
   */
