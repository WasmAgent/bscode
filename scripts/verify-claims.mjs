...
for (const claim of claims) {
  ...
  // evidence_script may be a bare path or a full command (e.g. "bun test path/to/file.ts --flag")
  // Extract the first token that looks like a file path to check.
  if (claim.evidence_script) {
    const tokens = claim.evidence_script.split(/\s+/);
    const fileLike = tokens.find((t) => /\.(ts|mjs|js|py)$/.test(t));
    const pathToCheck = fileLike ?? tokens[0];
    const scriptPath = resolve(ROOT, pathToCheck);
    if (!existsSync(scriptPath)) {
      errors.push(`${label}: evidence_script file '${pathToCheck}' does not exist on disk`);
    }
  }
