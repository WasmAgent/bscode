...
>
> 状态：分析阶段，不动 bscode 代码。等 evomerge SFT v5 完成 + wasmagent
...
- **goal 路线（wasmagent `GoalAgent`）**：framework 接管 verify 时机，
...
| **Skills** | 无（wasmagent core 有，bscode 不直接用） | ⚪ 现阶段不需要 |
...
| **Verifier sub-agent** | `visualVerifier.ts` + `visionJudge.ts` —— **bscode 比 wasmagent 多一层 visual verifier** | ✅+ 超出 |
| **`/goal` 原语** | 无（隐含在 maxSteps + tool call patterns） | ⚠️ 可加 |
| **L1 / L2 / L3 分层** | 部分（fan-out 是横向，不是纵向 tier） | ⚠️ 可加 |
| **IPT 反 reward-hacking** | 无 | ⏳ 等 wasmagent 发布版进入 |
...
**代价**：
- 需要 bscode 升级到下一个 wasmagent 发布版（含 `GoalAgent`）
- ~50 LOC 的 adapter

**建议时机**：等 wasmagent `GoalAgent` 发版本号（当前在 main 但还没 release）+ evomerge v5 SFT 跑完 + 验证 `GoalAgent` 在 1.7B 模型上的真实表现。
...
wasmagent memory eval 一起 publish。
...
`visionJudge.ts` 是 bscode 跟 wasmagent 的差异化优势之一（wasmagent 只有
...
但这必须等 wasmagent 把 `GoalAgent` 进 release tag，bscode 不能锁
`workspace:*`。
...
bscode 改动的前置：
1. wasmagent-js cut 一个 release tag 含 `GoalAgent` (当前 commit `e324fb9`)
...
- wasmagent-js [Loop Engineering vs wasmagent-js mapping](../../wasmagent-js/docs/strategy/loop-engineering-vs-wasmagent.md)
- wasmagent-js `GoalAgent` 源代码：`packages/core/src/agents/GoalAgent.ts`
- wasmagent-js `IPT` 源代码：`packages/evals-runner/src/stats/ipt.ts`
- bscode [B2 — Closed-loop validation](./B2-validation-loop.md)
- 用户给的 Loop Engineering 文档：`~/Downloads/Loop-Engineering-循环工程-最终整合版.md`
