...
| Training data pipeline | B2 build signals + C3 visual assertions feed `RolloutRanker` in wasmagent-js, which ranks rollout branches by objective score. Ranked branches are exported as DPO/PPO JSONL by [trace-pipeline](https://github.com/WasmAgent/trace-pipeline) (`evomerge` Python package) for downstream RL post-training. |
