...
| **Visual verification channel (C3)** | `apps/worker/src/build-results.test.ts` + `tools/build-result.test.ts` (10 tests) | `BuildResultSnapshot.visual` carries `consoleErrors`, `uncaughtErrors`, `domProbes`, and `rendersNonEmpty`; `read_build_result` surfaces these to the agent; thumbnail data URLs are NEVER inlined into agent context (size discipline) |
...
| **C3** Visual verification (`BuildResultSnapshot.visual`) | The browser `useWebContainer` hook schedules a visual check 1.5 s after `server-ready` and POSTs `{ consoleErrors, uncaughtErrors, domProbes, rendersNonEmpty, thumbnailDataUrl }`. The agent reads these signals via `read_build_result` and self-corrects on render errors / blank pages without seeing the (potentially huge) thumbnail bytes. |
